...
static void		ft_infopyr_fix(t_arena *arena, t_corvisu *visual)
{
	...
	while (p < arena->nb_pyrs)
	{
		...
		mvwprintw(visual->win_info_pyrs, 1 + 10 * p, 2, "Player %d", arena->tab_pyr[p].pyr_nbr);
		mvwprintw(visual->win_info_pyrs, 3 + 10 * p, 5, "Name : %s", arena->tab_pyr[p].pyr_name);
...
void		ft_info_game(t_arena *arena, t_corvisu *visual)
{
	...
	//mvwprintw(visual->win_info_game, 1, 100,  "%2d", arena->last_live_pyr);
}
