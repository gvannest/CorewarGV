...
{
	if ((ft_strnequ(line, COMMENT_CMD_STRING, 8) && info->comment_f == 0) || info->comment_f < 0)
	{
		info->quote = (info->quote == 2) ? 0 : info->quote;
		ft_comment(&(*info), line);
		if (info->comment_f < 0 && info->quote != 2)
			info->comment[ft_strlen(info->comment)] = '\n';
	}
	else if ((ft_strnequ(line, NAME_CMD_STRING, 5) && info->name_f == 0) || info->name_f < 0)
	{
		info->quote = (info->quote == 2) ? 0 : info->quote;
		ft_name(&(*info), line);
	//	printf("*** DEBUG ***\nQuote = %d\n***       ***\n", info->quote);
		if (info->name_f < 0 && info->quote != 2)
